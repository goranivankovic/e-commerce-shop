...
import womenJacket from '../../Img/kid/jeans.jpg'
...
function KidCard3() {
  ...
  const ik3 = useSelector((state) => state.counter.ik2)
  const kp3 = useSelector((state) => state.counter.kp2)
    ...
    <div>
       ...
       <div className={womenCardStyle.secondDivImage}>
        ...
        <div className={womenCardStyle.secondDivImageText}>
          ...
          <div>{   row4Item1  } ||  {  row4Item3  }<hr></hr> </div>
          <div> {ik3} <hr></hr> </div>

         <div>  {kp3} {valuta} <hr></hr></div>  
        ...
        setBtnValue('S')
         ...
         <div>id : 01054090f21111ec932b15 <hr></hr></div>
     ...
     <Link to="/shop">  <button style={{width:"100%"}}  disabled={!laz} onClick={()=>{ 
             ...
             dispatch(incrementBankAcount(kp3))

             dispatch(nizArray({id:Date.now().toString(), imgLink:"/kid/jeans",  imgName :"/Img/kid/jeans.jpg", name:ik3, size:btnValue,price:kp3,}) )
